...
class changePwd extends Component {
  ...
  handleSubmit() {
    ...
    if (!this.pwd.value || !this.pwdConfirm.value)
    {
      ...
      this.setState(Object.assign({}, this.state, {
        isToastShow: true,
        toastContent: <FormattedMessage id="app.account.difference" />,
      }));
      return false;
    } else {
      api({
      ...
      .then((res) => {
        ...
        self.setState(Object.assign({}, this.state, {
          isToastShow: true,
          toastContent: <FormattedMessage id="app.account.changeSuccess" />,
        }));
        setTimeout(function () {
          self.setState({ isToastShow: false });
          window.location = './account.html#/signIn';
        }, 3000);
      })
      .catch((err) => {
        self.setState(Object.assign({}, this.state, {
    ...
    return (
      <form className="changePwd">
        <div className="form-control">
          <input
            type="password" name="pwd" placeholder={this.context.intl.messages['app.account.changePwd.inputPwd']}
        ...
        <div className="form-control">
          <input
            type="password" name="pwdConfirm" placeholder={this.context.intl.messages['app.account.changePwd.inputPwdAgain']}
        ...
        <div className="form-control form-control-border">
        {
          ...
          ? <Button type="primary" onClick={this.handleSubmit} ><FormattedMessage id="app.account.changePwd.submit" /></Button>
          : <Button type="primary" className="btn-disabled"><FormattedMessage id="app.account.changePwd.submit" /></Button>
        }
        </div>
        <Toast
